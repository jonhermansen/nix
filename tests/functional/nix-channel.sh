...
cp "${config_nix}" dependencies.nix dependencies.builder*.sh $TEST_ROOT/bsdexprs/
ln -s dependencies.nix $TEST_ROOT/bsdexprs/default.nix
(cd $TEST_ROOT && tar cvf - nixexprs) | bzip2 > $TEST_ROOT/foo/bsdexprs.tar.bz2
...
nix-channel --add file://$TEST_ROOT/foo/bsdexprs.tar.bz2 bar
nix-channel --update
