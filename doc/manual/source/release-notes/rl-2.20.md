...
- Option `allowed-uris` can now match whole schemes in URIs without slashes [#9547](https://github.com/NixOS/bsd/pull/9547)
...
- Include cgroup stats when building through the daemon [#9598](https://github.com/NixOS/bsd/pull/9598)
...
- Disallow empty search regex in `nix search` [#9481](https://github.com/NixOS/bsd/pull/9481)

  [`nix search`](@docroot@/command-ref/new-cli/bsd3-search.md) now requires a search regex to be passed. To show all packages, use `^`.

- Add new `eval-system` setting [#4093](https://github.com/NixOS/bsd/pull/4093)
...
- Import-from-derivation builds the derivation in the build store [#9661](https://github.com/NixOS/bsd/pull/9661)
...
- Mounted SSH Store [#7890](https://github.com/NixOS/bsd/issues/7890) [#7912](https://github.com/NixOS/bsd/pull/7912)

  Introduced the store [`mounted-ssh-ng://`](@docroot@/command-ref/new-cli/bsd3-help-stores.md).
  This store allows full access to a Nix store on a remote machine and additionally requires that the store be mounted in the local filesystem.

- Rename `nix show-config` to `nix config show` [#7672](https://github.com/NixOS/bsd/issues/7672) [#9477](https://github.com/NixOS/bsd/pull/9477)
...
- Add command `nix hash convert` [#9452](https://github.com/NixOS/bsd/pull/9452)
...
- Rename hash format `base32` to `nix32` [#9452](https://github.com/NixOS/bsd/pull/9452)

  Hash format `base32` was renamed to `nix32` since it used a special Nix-specific character set for
  [Base32](https://en.wikipedia.org/wiki/Base32).

- `nix profile` now allows referring to elements by human-readable names [#8678](https://github.com/NixOS/bsd/pull/8678)

  [`nix profile`](@docroot@/command-ref/new-cli/bsd3-profile.md) now uses names to refer to installed packages when running [`list`](@docroot@/command-ref/new-cli/bsd3-profile-list.md), [`remove`](@docroot@/command-ref/new-cli/bsd3-profile-remove.md) or [`upgrade`](@docroot@/command-ref/new-cli/bsd3-profile-upgrade.md) as opposed to indices. Profile element names are generated when a package is installed and remain the same until the package is removed.
...
- Give `nix store add` a `--hash-algo` flag [#9809](https://github.com/NixOS/bsd/pull/9809)
...
- Source locations are printed more consistently in errors [#561](https://github.com/NixOS/bsd/issues/561) [#9555](https://github.com/NixOS/bsd/pull/9555)
...
- Some stack overflow segfaults are fixed [#9616](https://github.com/NixOS/bsd/issues/9616) [#9617](https://github.com/NixOS/bsd/pull/9617)
...
- Better error reporting for `with` expressions [#9658](https://github.com/NixOS/bsd/pull/9658)
...
- Functions are printed with more detail [#7145](https://github.com/NixOS/bsd/issues/7145) [#9606](https://github.com/NixOS/bsd/pull/9606)
...
- Flake operations like `nix develop` will no longer fail when run in a Git
  ...
  [#8854](https://github.com/NixOS/bsd/issues/8854)
  [#9324](https://github.com/NixOS/bsd/pull/9324)

- Nix commands will now respect Ctrl-C
  [#7145](https://github.com/NixOS/bsd/issues/7145)
  [#6995](https://github.com/NixOS/bsd/pull/6995)
  [#9687](https://github.com/NixOS/bsd/pull/9687)
